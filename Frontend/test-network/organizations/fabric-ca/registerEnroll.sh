...
function createOrg1() {
  ...
  set -x
  fabric-ca-client register --caname ca-org1 --id.name org1admin --id.secret org1adminpw --id.type admin --id.attrs 'age=20:ecert,email=jonas@gmail.com:ecert' --tls.certfiles "${PWD}/organizations/fabric-ca/org1/tls-cert.pem"
  { set +x; } 2>/dev/null
  ...
  # Part of the testing of performance. Not in the initial file. We register 10 additional peers.
  for i in {1..10}
  do 
    ...
    mkdir -p "${PWD}/organizations/peerOrganizations/org1.example.com/msp/tlscacerts"
    cp "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer$i.org1.example.com/tls/tlscacerts/"* "${PWD}/organizations/peerOrganizations/org1.example.com/msp/tlscacerts/ca$i.crt"

    mkdir -p "${PWD}/organizations/peerOrganizations/org1.example.com/tlsca"
    cp "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer$i.org1.example.com/tls/tlscacerts/"* "${PWD}/organizations/peerOrganizations/org1.example.com/tlsca/tlsca.org1.example.com-cert$i.pem"

    mkdir -p "${PWD}/organizations/peerOrganizations/org1.example.com/ca"
    cp "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer$i.org1.example.com/msp/cacerts/"* "${PWD}/organizations/peerOrganizations/org1.example.com/ca/ca.org1.example.com-cert$i.pem"

  done 
  # End of custom part added for performance testing.
...
function createOrg2() {
  ...
  infoln "Registering the org admin"
  set -x
  fabric-ca-client register --caname ca-org2 --id.name org2admin --id.secret org2adminpw --id.type admin --id.attrs 'age=28:ecert,email=thomas@gmail.com:ecert' --tls.certfiles "${PWD}/organizations/fabric-ca/org2/tls-cert.pem"
  { set +x; } 2>/dev/null
